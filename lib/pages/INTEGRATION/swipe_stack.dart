...
class SwipeStackState extends State<SwipeStack> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _animationController.addListener(() {
      ...
      if (_animationController.status == AnimationStatus.forward) {
        ...
        if (_animationType != 1 && _animationAngle != null) _angle = _animationAngle.value;
    ...
    _animationController.addStatusListener((AnimationStatus animationStatus) {
      ...
      if (animationStatus == AnimationStatus.completed) {
        ...
        if (_animationType != 3 && _animationType != 0) {
          ...
          if (widget.onRewind != null) widget.onRewind!(widget.children.length - 1, _history[_history.length - 1]["position"]);
        ...
        if (_animationType != 0 && _animationType != 3) {
          ...
          if (widget.onSwipe != null) widget.onSwipe!(widget.children.length, _currentItemPosition);


          if (widget.children.length == 0 && widget.onEnd != null) widget.onEnd!();
      ...
      return Visibility(
        ...
        child: Positioned(
            ...
            child: Transform.scale(
                ...
                child: Container(constraints: constraints, child: widget.children[index].builder(SwiperPosition.None, 0)))),
    ...
    return Positioned(
      ...
      child: GestureDetector(
          ...
          child: Transform.rotate(
            ...
            child: Container(constraints: constraints, child: widget.children[index].builder(_currentItemPosition, _progress)),
  ...
  void _onPandEnd(_) {
    ...
    if (_progress < widget.threshold) {
      ...
      _animationX =
          Tween<double>(begin: _left, end: _baseContainerConstraints.maxWidth * (_left < 0 ? -1 : 1)).animate(_animationController);
  ...
  void _goFirstPosition() {
    ...
    if (widget.maxAngle > 0) _animationAngle = Tween<double>(begin: _angle, end: 0.0).animate(_animationController);
  ...
  void swipeLeft() {
    ...
    if (widget.children.length > 0 && _animationController.status != AnimationStatus.forward) {
      ...
      if (widget.maxAngle > 0) _animationAngle = Tween<double>(begin: 0, end: _maxAngle * 0.7).animate(_animationController);
  ...
  void swipeRight() {
    ...
    if (widget.children.length > 0 && _animationController.status != AnimationStatus.forward) {
      ...
      if (widget.maxAngle > 0) _animationAngle = Tween<double>(begin: 0, end: (_maxAngle * 0.7) * -1).animate(_animationController);
  ...
  void rewind() {
    ...
    if (_history.length > 0 && _animationController.status != AnimationStatus.forward) {
      ...
      if (widget.maxAngle > 0) _animationAngle = Tween<double>(begin: lastHistory["angle"], end: 0).animate(_animationController);
